...
if __name__ == '__main__':
    ...
    with accelerator.main_process_first():
        ...
        if 'input_ids_0' in train_dataset[0]:
            array_size = len(train_dataset[0]['input_ids_0'])
        elif 'input_ids' in train_dataset[0]:
            array_size = len(train_dataset[0]['input_ids'])
        else:
            raise ValueError('Unknown dataset format')

    def cell_autom_collate_fn(batch, valid=False):
        for i, b in enumerate(batch):
            if repeat_state:
                batch[i] = {
                    'input_ids': [
                        i for t in range(steps-1) if f'input_ids_{t}' in b \
                ...
                batch[i] = {
                    'input_ids': [
                        i for t in range(steps) if f'input_ids_{t}' in b \
