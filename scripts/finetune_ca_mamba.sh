...
BACKBONE_CLS=baselines.mamba.mamba_act:AdaptiveMambaForCausalLM
...
cd ../..
MODEL_CFG=~/rmt/wip/base_models/gptconfigs/mamba_${NUM_LAYERS}l${NUM_LAYERS}hd${DIM}.json
...
accelerate launch --num_processes $NP --config_file  ./accelerate.yaml --main_process_port 29502 run_finetuning_cell_autom.py \
        --task_name $TASK_NAME \
        --model_path ../runs/lm_long/mamba/${TASK_NAME}/$MODEL_NAME/lr${LR}_${SCHEDULER}_dmem${D_MEM}_${INPUT_SEQ_LEN}-${MAX_N_SEGMENTS}x${INPUT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_iters${ITERS}_${SEGMENT_ORDERING}_bptt-${K2}_act$ACT_TYPE_shift$SHIFT/run_$N \
        --model_cfg $MODEL_CFG \
